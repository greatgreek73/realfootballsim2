...
def get_players(request, match_id):
    ...
    print(f"Returning {len(player_data)} players for match {match_id}")  # Отладочный вывод
...
@login_required
@require_http_methods(["GET"])
def get_team_selection(request, match_id):
    match = Match.objects.get(pk=match_id)
    if request.user.club not in [match.home_team, match.away_team]:
        return JsonResponse({'error': 'Unauthorized'}, status=403)
    
    team_selection = TeamSelection.objects.filter(match=match, club=request.user.club).first()
    if team_selection:
        return JsonResponse({'selection': team_selection.selection})
    return JsonResponse({'selection': None})
