...
def get_locale_from_country_code(country_code):
    supported_locales = Faker.locales  # Получаем список поддерживаемых локализаций
    locale = f"{country_code.lower()}_{country_code.upper()}"
    return locale if locale in supported_locales else 'en_US'
...
# Дополнительные функции или классы могут быть добавлены здесь, если это требуется для проекта
