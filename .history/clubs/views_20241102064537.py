...
class CreateClubView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.debug("Club instance created without saving")
            
            # Устанавливаем владельца и флаг is_bot
            club.owner = self.request.user
            club.is_bot = False
            club._skip_clean = True  # Пропускаем валидацию
            logger.debug(f"Owner set to: {self.request.user.username}")
