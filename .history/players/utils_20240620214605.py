...
CLASS_MODIFIERS = {1: 0.8, 2: 0.9, 3: 1.1, 4: 1.2}
...
def apply_class_modifier(stats, player_class):
    modifier = CLASS_MODIFIERS[player_class]
    return {attr: max(1, min(100, int(value * modifier))) for attr, value in stats.items()}
...
def generate_player_stats(position, player_class):
    ...
    return apply_class_modifier(stats, player_class)