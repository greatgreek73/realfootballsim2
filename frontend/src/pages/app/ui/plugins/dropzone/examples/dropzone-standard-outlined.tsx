...
export default function DropzoneStandardOutlined() {
  return (
    <Card>
      <CardContent>
        ...
        <Box component="form" className="mb-0 flex cursor-pointer flex-col">
          <DropzoneStandardOutlinedEmpty />
          <DropzoneStandardOutlinedFilled />
        </Box>
      </CardContent>
    </Card>
  );
...
function DropzoneStandardOutlinedEmpty() {
  ...
  const thumbs = files.map((file) => (
    <Box
      key={file.name}
      className="bg-grey-25 flex-non flex w-80 cursor-default flex-row items-start rounded-sm p-1"
      onClick={(event) => {
  ...
  return (
    <FormControl className="outlined" variant="standard" size="small">
      ...
      <Box
        ...
        className="border-grey-200 hover:border-grey-500 flex min-h-[90px] flex-row flex-wrap gap-2.5 rounded-md border p-4 transition-all"
      >
        <input {...getInputProps()} />
        {files.length > 0 ? (
          thumbs
...
function DropzoneStandardOutlinedFilled() {
  const [files, setFiles] = useState<(any & { preview: string })[]>([
    {
